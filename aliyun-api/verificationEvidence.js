export function extractVerificationEvidence(institutionName, rawResults = []) {
  const seen = new Set();
...
export function buildRawResultItems(institutionName, rawResults = []) {
  return rawResults
    .map((item) => normalizeSearchResult(item))
    .filter(Boolean)
    .map((item) => {
...
export function buildBusinessProfile(officialRegistry) {
  return {
...
function normalizeSearchResult(item) {
  ...
  const result = wrapper.result && typeof wrapper.result === 'object' ? wrapper.result : {};
...
function buildRiskSignal(category, text) {
...
function isInstitutionMatch(institutionName, text) {
...
function detectRiskCategories(text) {
  return [
...
function getRawResultRelevance({ institutionMatched, riskTags }) {
...
function buildKeywordDiagnostics(rawResults, rawResultItems, suppliedDiagnostics = []) {
...
function getVerificationJudgment(status, riskTags, evidenceCount) {
...
function getRiskLevel(judgment) {
  return ({
    clear: 'low',
    pending: 'unknown',
    skipped: 'unknown',
...
function getVerificationConclusion(judgment, evidenceCount, riskTags, errorMessage) {
...
function getVerificationRecommendation(judgment) {
  return ({
...
function getSuggestedPublicCreditStatus(judgment) {
  return ({
    clear: 'normal',
    review_required: 'medium',
