...
- 申请额度达到配置的高额度阈值以上，默认阈值为 `VITE_DEEP_VERIFICATION_HIGH_LIMIT=50000`。
- 智谱联网核验发现需复核风险线索。
- 合作阶段未满 6 个月。
- 当前评估结果需要特批。

未配置供应商 Key 前，这个开关只做提示，不发起授权工商核验。
...
- 人工确认日志可以记录业务人员主动采用的公共信用状态建议，并支持上传截图 / PDF 作为证据附件，但不由后台自动改写评分、红线或授信结论。
- 证据附件由 Edge Function 写入私有 Supabase Storage bucket，前端只保存附件元数据和短期 signed URL，不暴露 service role。
- 生产阶段还需要接入真实登录身份、复核人权限和审批流。

## 后续建议

后续可以继续做：

1. Provider 抽象：把智谱调用拆成独立 adapter，方便接入其他 AI API。
2. 附件闭环增强：继续增加附件删除、预览缩略图和登录后的访问控制。
3. 审批闭环：把复核人扩展为特批审批人、审批意见和审批状态流转。
